...
#  puts [format $format {SQLite 2.4 (cache=100):} $t]
  ...
  set t [expr {[lindex $t 0]/1000000.0}]
  puts [format $format {SQLite 3.11:} $t]
  exec sync; after $delay;
  set t [time "exec sqlite3 sns.db <$sqlfile" 1]
  set t [expr {[lindex $t 0]/1000000.0}]
  puts [format $format {SQLite 3.11 (nosync):} $t]
...
exec sqlite3 s2kb.db <2kinit.sql
...
close $fd
exec sqlite3 sns.db <nosync-init.sql
exec sqlite3 snsb.db <nosync-init.sql
